...
const renderFilesTable = (paths, fontName) => {
  ...
  exportCmd.getFiles(paths, fontName).map((item) => {
...
program
  .command('export <fontName> <icomoonZipFile> <preProcessorPath> <cssPath> <fontsPath> <docsPath> <ligaPath> ')
  .alias('b')
  ...
  .action((fontName, icomoonZipFile, preProcessorPath, cssPath, fontsPath, docsPath, ligaPath, options) => {
    const paths = {
      fonts: path.resolve(fontsPath),
      preProcessor: path.resolve(preProcessorPath),
      css: path.resolve(cssPath),
      docs: path.resolve(docsPath),
      liga: path.resolve(ligaPath)
    }

    if (options.force === true) {
      return exportCmd.cmd(icomoonZipFile, paths)
    }
...
program
  .command('clean <preProcessorPath> <cssPath> <fontsPath> <docsPath> <ligaPath>')
  .alias('c')
  .description('Cleans previously generated files.')
  .option('--force', 'Executes the command straight away without user confirmation.')
  .action((preProcessorPath, cssPath, fontsPath, docsPath, ligaPath, options) => {
    const paths = [
      path.resolve(preProcessorPath),
      path.resolve(cssPath),
      path.resolve(fontsPath),
      path.resolve(docsPath),
      path.resolve(ligaPath)
    ]
